...
class HiRadixCache(RadixCache):
    ...
    def check_session_cache_events(self):
        ...
        for session_id, (node, copy_event) in list(self.ongoing_session_append.items()):
            print("check session cache event", session_id, node.key, node.value, node.host_value, copy_event)
    ...
    def prefetch_from_session_cache(
        ...
        if old_kv_cache is None:
            prefetch_length = len(new_input_tokens) - len(new_input_tokens)
            new_input_tokens = new_input_tokens[:prefetch_length]
        else:
            prefetch_length = sum(seg['token_length'] for seg in old_kv_cache) - starting_offset
            print("prefetch param", old_kv_cache, prefetch_length, starting_offset)
    ...
    def check_session_prefetch_progress(self, session_id: str) -> bool:
        ...
        if operation.done == True:
            print("check session prefetch progress", session_id, operation.token_ids, len(operation.token_ids),
    ...
    def write_backup_session(
        ...
        starting_offset = new_kv_cache[0]['token_start'] if new_kv_cache else 0
